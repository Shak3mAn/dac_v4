...
export const PartNameSearch = (props: Props) => {
    ...
    const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const results: InventoryItem[] = inv_data.filter(
            (result) =>
                result.itemName.toLowerCase().includes(term.toLowerCase())
        );
    ...
    const handleResultClick = (result: InventoryItem) => {
        setSearchTerm(result.itemName);
        setShowDropdown(false);

      invtItem.onAddTempInventoryItem(result);

        toast({
            title: "New Service: Part selected",
            description: `Part Name: ${result.itemName}`,
        })
    }
  return (
    <div>
