...
const getInvoiceTemplate = async (templateId: number) => {
    ...
    const componentName = `invoice-template${templateId}`;

    try {
        let myModule = await import(
            `@/components/general/template/invoice-pdf/${componentName}`
        );
...
/**
 * Dynamically imports and retrieves an job card template React component based on the provided template ID.
 *
 * @param {number} templateId - The ID of the job card template.
 * @returns {Promise<React.ComponentType<any> | null>} A promise that resolves to the job card template component or null if not found.
 * @throws {Error} Throws an error if there is an issue with the dynamic import or if a default template is not available.
 */
const getJobCardTemplate = async (templateId: number) => {
    // Dynamic template component name
    const componentName = `job-card-template${templateId}`;

    try {
        let myModule = await import(
            `@/components/general/template/job-card-pdf/${componentName}`
        );
        return myModule.default;
    } catch (err) {
        console.error(`Error importing template ${componentName}: ${err}`);
...
/**
 * Dynamically imports and retrieves an gate pass template React component based on the provided template ID.
 *
 * @param {number} templateId - The ID of the gate pass template.
 * @returns {Promise<React.ComponentType<any> | null>} A promise that resolves to the gate pass template component or null if not found.
 * @throws {Error} Throws an error if there is an issue with the dynamic import or if a default template is not available.
 */
// const getGatePassTemplate = async (templateId: number) => {
//     // Dynamic template component name
//     const componentName = `gate-pass-template${templateId}`;

//     try {
//         let myModule = await import(
//             `@/components/general/template/gate-pass-pdf/${componentName}`
//         );
//         return myModule.default;
//     } catch (err) {
//         console.error(`Error importing template ${componentName}: ${err}`);
