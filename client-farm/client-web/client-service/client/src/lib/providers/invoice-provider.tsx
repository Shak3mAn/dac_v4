...
import { exportInvoice } from "@/service/invoice/client/exportInvoice";

import {
  FORM_DEFAULT_VALUES,
  GENERATE_INVOICE_PDF_API,
  SHORT_DATE_OPTIONS,
} from "@/lib/utils/variables";

import { ExportTypes, InvoiceType } from "../types/types";
...
export const InvoiceContextProvider = ({
  ...
  const generatePdf = useCallback(async (data: InvoiceType) => {
    ...
    try {
      const response = await fetch(GENERATE_INVOICE_PDF_API, {
        method: "POST",
